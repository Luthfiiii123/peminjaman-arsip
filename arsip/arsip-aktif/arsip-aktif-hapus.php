...
if (isset($_GET['id'])) {
    ...
    ============================================== */
    $query_cek = mysqli_query($db, "SELECT file_pdf FROM arsip_aktif WHERE id_arsip_aktif = '$id'");
    ...
    ============================================== */
    $hapus = mysqli_query($db, "DELETE FROM arsip_aktif WHERE id_arsip_aktif = '$id'");

    if ($hapus) {
        // Berhasil -> Kembali ke halaman arsip (bukan index login)
        echo "<script>alert('Data Berhasil Dihapus!'); window.location='arsip-aktif.php';</script>";
    } else {
        // Gagal
        echo "<script>alert('Gagal menghapus data: " . mysqli_error($db) . "'); window.location='arsip-aktif.php';</script>";
    }
