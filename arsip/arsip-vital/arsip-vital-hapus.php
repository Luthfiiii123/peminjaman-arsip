...
if (isset($_GET['id'])) {
    ...
    ============================================== */
    $query_cek = mysqli_query($db, "SELECT file_pdf FROM arsip_vital WHERE id_arsip_vital = '$id'");
    ...
    ============================================== */
    $hapus = mysqli_query($db, "DELETE FROM arsip_vital WHERE id_arsip_vital = '$id'");

    if ($hapus) {
        // Berhasil -> Kembali ke halaman arsip (bukan index login)
        echo "<script>alert('Data Berhasil Dihapus!'); window.location='arsip-vital.php';</script>";
    } else {
        // Gagal
        echo "<script>alert('Gagal menghapus data: " . mysqli_error($db) . "'); window.location='arsip-vital.php';</script>";
    }

} else {
    // Jika tidak ada ID, kembalikan saja
    header("Location: arsip-vital.php");
}
?>